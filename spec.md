...
> 文件狀態：Draft v0.4（需求已確認；v0.4 修訂跨季頁為全季度列表與分享展開規則）
> 建立日期：2026-07-11
...
| `seasonIds` | string[] | 快照涉及的季度；單季恰為一個，跨季為要展開比較的季度集合（一個以上） |
...
1. 跨季頁列出季度選單中的所有季度區塊；預設展開最新兩季、其餘收合。由設定或分享還原時，改為展開 snapshot 記錄的季度、其餘收合。每季區塊可展開或收合，資料於需要時（展開或還原）才載入。
2. 頁面上方只有一份跨季共用的彙整清單，包含所有已載入季度中已選觀看狀態或已有名次的作品；有名次者先排序，未排名者其後依來源順序排列。
...
8. 由分享連結開啟跨季頁時，顯示上方彙整清單，並展開 snapshot 記錄的季度、其餘季度收合；收件者之後可自行展開或再次收合。
...
3. 跨季分享連結開啟時顯示共用彙整清單，並展開分享記錄的季度、其餘收合，收件者可再展開或收合。
4. 跨季清單依規則在上方排序，收合季度不影響結果。
...
4. 跨季頁列出所有季度，預設展開最新兩季、其餘收合，並共用一份上方彙整清單；分享與設定記錄實際展開比較的季度集合。
5. 跨季同名、延期或跨季播出作品不特別判定，不維護人工 alias/canonical ID；先以名稱配對，名稱命中的項目都可能套用同一狀態並重複顯示。
6. 單季 preset 綁定原季度，動畫狀態以名稱還原；跨季 preset 綁定其展開比較的季度集合。
7. 分享連結可由收件者繼續修改並產生自己的連結。
8. 開啟跨季分享時顯示上方彙整清單，並展開分享記錄的季度、其餘收合；收件者可自行展開／收合。
